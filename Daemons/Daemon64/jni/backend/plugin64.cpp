...
int main()
{
    ...
    while (isBeta || (receive((void*)&request) > 0)) {
        ...
        if (strstr(name, "BP_PlayerPawn")||strstr(name, "BP_PlayerCharacter")||strstr(name, "PlanET_FakePlayer")||strstr(name, "BP_PlayerCharacter_PlanX_C")||strstr(name, "PlanX_PlayerCharacter_AI_C")) {
                ...
                if (getI(pBase + Offsets64::bDead)) //bool bDead;//(ByteOffset: 0, ByteMask: 1, FieldMask: 1)[Offset: 0xd0c, Size: 0x1]
                    continue;
                ...
                //me
                uintptr_t me = getI(pBase + Offsets64::Role); //byte Role;//[Offset: 0x140, Size: 0x1]
                ...
                if (data->ActorTeamID == myteamID && myteamID <= 100)
                    continue;
                pvm(getA(pBase + Offsets64::ObserverCameraFPPMode) + Offsets64::ObserverLevel, &exyz, sizeof(exyz)); //150
                ...
                if (data->Distance > 600.0f)
                    continue;
                pvm(pBase + Offsets64::bIsAI, &data->isAI, sizeof(data->isAI)); //Class: UAECharacter.Character.Pawn.Actor.Object -> //bool bIsAI;//(ByteOffset: 0, ByteMask: 1, FieldMask: 255)[Offset: 0x960, Size: 0x1]
...
uintptr_t getEntityAddr(uintptr_t base)
{
    //return getA(getA(getA(getA(getA(base + GWorld) + 0x68) + 0x18)) + 0x30) + 0xb0;
...
ActorBone getActorBone(uintptr_t pBase, struct D3DMatrix viewMatrix)
{
    ...
    boneAddr = getA(boneAddr + Offsets64::MinLOD); //Class: StaticMeshComponent.MeshComponent.PrimitiveComponent.SceneComponent.ActorComponent.Object -> //int MinLOD;//[Offset:
    //neck 0
...
PlayerWeapon getPlayerWeapon(uintptr_t base) {
    PlayerWeapon p;
    uintptr_t addr[3];
    pvm(getA(base + Offsets64::ActorChildren), addr, sizeof(addr)); //Class: Actor.Object -> //Actor*[] Children;//[Offset: 0x1a8, Size: 0x10] -> Ver: 2.1.0

    if (isValid64(addr[0]) && getI(addr[0] + Offsets64::DrawShootLineTime) == 2) { //Class: WeaponEntity.WeaponLogicBaseComponent.ActorComponent.Object -> Offset
        p.playerWeapon = true;
        p.id = getI(getA(addr[0] + Offsets64::WeaponEntityComp) + Offsets64::UploadInterval); //Class: STExtraWeapon.Actor.Object -> //WeaponEntity* WeaponEntityComp;//[Offset: 0x708, Size: 0x8] + Class: BulletHitInfoUploadComponent.WeaponLogicBaseComponent.ActorComponent.Object -> //float UploadInterval;//[Offset: 0x170, Size: 0x4]
        p.ammo = getI(addr[0] + Offsets64::CurBulletNumInClip); //Class: STExtraShootWeapon.STExtraWeapon.Actor.Object -> //int CurBulletNumInClip;//[Offset: 0xd88, Size: 0x4] -> Ver: 2.1.0
    } else if (isValid64(addr[1]) && getI(addr[1] + Offsets64::DrawShootLineTime) == 2) {//Class: WeaponEntity.WeaponLogicBaseComponent.ActorComponent.Object
        p.playerWeapon = true;
        p.id = getI(getA(addr[1] + Offsets64::WeaponEntityComp) + Offsets64::UploadInterval); //Class: STExtraWeapon.Actor.Object -> //WeaponEntity* WeaponEntityComp;//[Offset: 0x708, Size: 0x8] + Class: BulletHitInfoUploadComponent.WeaponLogicBaseComponent.ActorComponent.Object -> //float UploadInterval;//[Offset: 0x170, Size: 0x4]
        p.ammo = getI(addr[1] + Offsets64::CurBulletNumInClip); //Class: STExtraShootWeapon.STExtraWeapon.Actor.Object -> //int CurBulletNumInClip;//[Offset: 0xd88, Size: 0x4] -> Ver: 2.1.0
    } else if (isValid64(addr[2]) && getI(addr[2] + Offsets64::DrawShootLineTime) == 2) {//Class: WeaponEntity.WeaponLogicBaseComponent.ActorComponent.Object
        p.playerWeapon = true;
        p.id = getI(getA(addr[2] + Offsets64::WeaponEntityComp) + Offsets64::UploadInterval); //Class: STExtraWeapon.Actor.Object -> //WeaponEntity* WeaponEntityComp;//[Offset: 0x708, Size: 0x8] + Class: BulletHitInfoUploadComponent.WeaponLogicBaseComponent.ActorComponent.Object -> //float UploadInterval;//[Offset: 0x170, Size: 0x4]
        p.ammo = getI(addr[2] + Offsets64::CurBulletNumInClip); //Class: STExtraShootWeapon.STExtraWeapon.Actor.Object -> //int CurBulletNumInClip;//[Offset: 0xd88, Size: 0x4] -> Ver: 2.1.0
    }
