...
Java_com_skullshooter_ssplugin_app_services_WindowOverlay_DrawOn(JNIEnv *env, jclass , jobject espView, jobject canvas) {
    espOverlay = PerceptionEngine(env, espView, canvas);
    if (espOverlay.isValid()){
        DrawCanvas(espOverlay, espOverlay.getWidth(), espOverlay.getHeight());
    }
}
//com_skullshooter_ssplugin_app_services
extern "C" JNIEXPORT void JNICALL
Java_com_skullshooter_ssplugin_app_services_WindowOverlay_Close(JNIEnv *, jobject ) {
    Close();
}
extern "C" JNIEXPORT void JNICALL
Java_com_skullshooter_ssplugin_app_services_SsFlawMain_SettingValue(JNIEnv *, jobject , jint code, jboolean jboolean1) {
    switch((int)code){
        case 1:
...
Java_com_skullshooter_ssplugin_app_services_SsFlawMain_SizeConfiguarations(JNIEnv *env, jobject, jint feature, jint value) {
    switch (feature) {
        case 1: espScallingSize = value;
            break;
    }
}

extern "C" JNIEXPORT jboolean JNICALL
Java_com_skullshooter_ssplugin_app_services_WindowOverlay_getReady(JNIEnv *, jclass , int typeofgame) {
    int sockCheck = 1;
    if (!Create()) {
...
extern "C" JNIEXPORT void JNICALL
Java_com_skullshooter_ssplugin_app_services_SsFlawMain_SettingMemory(JNIEnv *,  jobject ,jint code, jboolean is_true) {
    switch ((int)code) {
        case 1: mRequest.memory.LessRecoil = is_true;
            break;
    }
}